...
function buildSitemap(scams) {
  ...
  const urls = [...staticPages, ...scamPages].map(u =>
    `  <url>
    <loc>${u.loc}</loc>
...
fs.writeFileSync(path.join(ROOT, 'sitemap.xml'), buildSitemap(scams), 'utf8');
console.log(`  wrote /sitemap.xml`);
fs.writeFileSync(path.join(ROOT, 'robots.txt'), buildRobots(), 'utf8');
console.log(`  wrote /robots.txt`);

console.log(`\n✓ Generated ${count} scam pages + index + sitemap + robots`);
